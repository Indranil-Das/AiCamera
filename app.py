...
st.title('Detect Anything And Everything')
        ...
        if upload_video_file is not None:
            ...
            if pred:
                while success:
                    ...
                    #label_map = label_map_util.load_labelmap(class_labels)
        ...
        if len(cam_options) != 0:
            if not cam_options == 'Select Channel':
                ...
                while True:
                    ...
                    bbox_list = []
                    results = model(img)
                    #count1(class_labels,img)
                    # Bounding Box
                    box = results.pandas().xyxy[0]
                    class_list = box['class'].to_list()

                     #[category_index.get(value) for index,value in enumerate(classes[0]) if scores[0,index] > 0.5]
                    # read class.txt
                    bytes_data = path_to_class_txt.getvalue()
                    class_labels = bytes_data.decode('utf-8').split("\n")
